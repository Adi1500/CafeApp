...
const Gosti = () => {
  ...
  const handlePrice = (cartArray) => {
    let ans = 0;
    cartArray.map((item) => (ans += (item.amount * item.cijena_skladiste)));
  ...
  const sendToNode = () => {
    ...
    {
      if((parseFloat(cart[i].kolicina_skladiste)-parseFloat(cart[i].amount)) < 0){
        window.alert("Žao nam je, ali ponestalo nam je "+ cart[i].ime_proizvoda +"")
        zaliha = false
      }
    }
    if(zaliha){
  ...
  return (
    ...
    `}
      ...
      <Accordion allowZeroExpanded="true">
        ...
        <AccordionItem>
          <AccordionItemHeading >
            <AccordionItemButton style={{backgroundImage: "url(" + Kafa + ")"}}>
              Topli napici
            </AccordionItemButton>
          </AccordionItemHeading>
          <AccordionItemPanel>
            {menuList.map(item => {
              if(item.podskupina === "topli")
                return (
                    ...
                    <GuestItems item={item} handleClick={handleClick} />
        ...
        <AccordionItem>
          <AccordionItemHeading>
            <AccordionItemButton style={{backgroundImage: "url(" + Sok + ")"}}>
              Sokovi
            </AccordionItemButton>
          </AccordionItemHeading>
          <AccordionItemPanel>
        ...
        <AccordionItem>
          <AccordionItemHeading>
            <AccordionItemButton style={{backgroundImage: "url(" + Alkohol + ")"}}>
              Alkohol
            </AccordionItemButton>
          </AccordionItemHeading>
          <AccordionItemPanel>
        ...
        <AccordionItem>
          <AccordionItemHeading>
            <AccordionItemButton style={{backgroundImage: "url(" + Hrana + ")"}}>
              Hrana
            </AccordionItemButton>
          </AccordionItemHeading>
          <AccordionItemPanel>
        ...
        <AccordionItem>
          <AccordionItemHeading>
            <AccordionItemButton style={{ backgroundImage: "url(" + Special + ")"}}>
              Posebne ponude
            </AccordionItemButton>
          </AccordionItemHeading>
          <AccordionItemPanel>
      ...
      <hr style={{backgroundColor: "red", height: "2px", border: "0.5px solid red"}}/>
        ...
        <div className="container content-section">
          {cart.map((item) => (
            <div className="cart-row" key={item.id}>
              ...
              <span className="cart-price cart-column">{item.cijena_skladiste}KM</span>
              <div className="cart-quantity cart-column">
                <input className="cart-quantity-input" id="qnt" type="number" defaultValue={item.amount} onChange={(e) => {handleChange(item, e); handlePrice(cart)}} />
                
                <button className="btn btn-danger" onClick={() => handleRemove(item.id)}>UKLONI</button>
              </div>
      ...
      <button className="purchaseBtn" onClick={sendToNode}>KUPI</button>
    </div>
